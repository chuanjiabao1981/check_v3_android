...
public class QuickCheckComposeActivity extends SherlockFragmentActivity
		implements QuickCheckEditorFragmentListener,
		QuickCheckViewerFragmentListener,
		QuickReportResolutionViewerFragmentListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.hasExtra(Constants.WHAT_ACTION)) {
			mQuickCheckAction = intent.getIntExtra(Constants.WHAT_ACTION,
					Constants.QUICK_CHECK_EDIT_NEW_ACTION);
		}
		...
		if (mQuickCheckAction == Constants.QUICK_CHECK_EDIT_NEW_ACTION) {
			...
			getSupportFragmentManager()
					.beginTransaction()
					.add(R.id.quick_check_compose_activity_container,
							firstFragment).commit();
		} else if (mQuickCheckAction == Constants.QUICK_CHECK_VIEW_ACTION) {
			QuickCheckListItemData qcListItemData = (QuickCheckListItemData) intent
					.getSerializableExtra(Constants.KEY_QUICK_CHECK_DATA);

			QuickCheckRspData qcReportData = CommonHelper
					.convertQuickCheckListItemData2RspData(qcListItemData);
			origData.putInt(Constants.WHAT_ACTION,
					Constants.QUICK_CHECK_VIEW_ACTION);
			origData.putInt(Constants.ORIG_REPORT_ID, qcReportData.getId());
			origData.putSerializable(Constants.KEY_QUICK_CHECK_DATA,
					qcReportData);
			SherlockFragment targetFragment = QuickCheckViewerFragment
					.newInstance(origData);
			...
			QuickCheckRspData qrRspData = (QuickCheckRspData) intent
					.getSerializableExtra(Constants.KEY_QUICK_CHECK_DATA);

			origData.putInt(Constants.WHAT_ACTION,
					Constants.QUICK_CHECK_VIEW_RSP_ACTION);
	...
	public void onProcessItemSelected() {
		// QuickCheckViewerFragment newFragment = new
		// QuickCheckViewerFragment();
		// Bundle args = new Bundle();
		//
		// FragmentTransaction transaction =
		// getSupportFragmentManager().beginTransaction();
		//
		// transaction.replace(R.id.quick_check_compose_activity_container,
		// newFragment);
	...
	public void onQuickCheckSubmitSuccess(Bundle qcRspData) {
		...
		if (fragMngr.getBackStackEntryCount() <= 0) {
			QuickCheckViewerFragment newFragment = QuickCheckViewerFragment
					.newInstance(qcRspData);
	...
	public void onQuickCheckEditMenuItemClicked(Bundle qrOrigData) {
		QuickCheckEditorFragment newFragment = QuickCheckEditorFragment
				.newInstance(qrOrigData);
	...
	public void onQuickCheckResolveMenuItemClicked(Bundle qrRslvData) {
		QuickReportResolutionEditorFragment newFragment = QuickReportResolutionEditorFragment
				.newInstance(qrRslvData);
	...
	public void onSubmitQuickReportResolutionSuccess(Bundle qrRslvRspData) {
		QuickReportResolutionViewerFragment newFragment = QuickReportResolutionViewerFragment
				.newInstance(qrRslvRspData);
